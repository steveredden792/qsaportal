...
use App\Livewire\PmrCatalogue;
use App\Livewire\PprCatalogue;
...
it('lists PPR provider reports with a from-price', function () {
    ...
    Report::factory()->ppr()->for($provider)->create(['slug' => 'ppr-prv-1000']);

    Livewire::test(PprCatalogue::class)
        ->assertSee('Acme Care')
        ->assertSee('£50.00')
        ->assertSee('ppr-prv-1000');
});

it('filters PPR by provider name live', function () {
...
it('lists PMR market reports with a from-price', function () {
    ...
    Report::factory()->pmr()->for($market)->create(['slug' => 'pmr-mkt-2000']);

    Livewire::test(PmrCatalogue::class)
        ->assertSee('Homelessness')
        ->assertSee('£50.00')
        ->assertSee('pmr-mkt-2000');
});

it('filters PMR by market name live', function () {
