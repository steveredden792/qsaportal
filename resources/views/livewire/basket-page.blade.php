<div class="space-y-6">
    <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <div class="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            <div>
                ...
                <h1 class="mt-2 text-3xl font-semibold text-slate-900">Your cart</h1>
    ...
    @endif
    @if (session('status'))
        <div class="rounded border border-brand/20 bg-brand/5 p-3 text-sm text-brand">{{ session('status') }}</div>
    @endif
    ...
    @else
            ...
            <table class="min-w-full divide-y divide-slate-200 text-sm">
                <tbody class="divide-y divide-slate-100">
                    @foreach ($reports as $report)
                        <tr wire:key="basket-{{ $report->id }}" class="hover:bg-slate-50">
                            <td class="px-4 py-3 font-medium text-slate-900">{{ $report->name }}</td>
                            <td class="px-4 py-3 text-right text-slate-700">{{ \App\Support\Money::format($price) }}</td>
                            <td class="px-4 py-3 text-right">
                                <button wire:click="remove({{ $report->id }})" class="text-sm font-medium text-red-600 transition hover:text-red-700">Remove</button>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
        ...
        <div class="flex flex-col gap-3 rounded-3xl border border-slate-200 bg-slate-50 p-6 shadow-sm sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                @else
                    <p class="mt-1 text-sm text-slate-600">You'll create an account (or log in) as part of checkout so your reports are saved to you.</p>
                @endauth
            </div>
